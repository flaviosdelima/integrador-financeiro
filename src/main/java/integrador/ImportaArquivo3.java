...
import java.io.*;
import javax.swing.*;
public class ImportaArquivo3 {

    public static void main(String args[])
    {
        final JFrame frame = new JFrame("Flavio Conveter .OFX 2 .XLS");

        final JFileChooser fc = new JFileChooser();
        fc.setMultiSelectionEnabled(true);
        fc.setCurrentDirectory(new File("C:\\tmp"));

        JButton btn2 = new JButton("Abrir Arquivo");
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (retVal == JFileChooser.APPROVE_OPTION) {
                    ...
                    for (int i = 0; i < selectedfiles.length; i++) {
                        sb.append(selectedfiles[i].getPath());//selectedfiles[i].getName()
                    }
                    //JOptionPane.showMessageDialog(frame, sb.toString());

                    try {
                        ImportaArquivo3 fileImportRN = new ImportaArquivo3();
                        //FileInputStream stream = new FileInputStream("/home/flavios/Documentos/projetos/MEC/nubank-2020-02.ofx");
                        FileInputStream stream = new FileInputStream( selectedfiles[0].getPath());
    ...
    public void importarCartaoCredito(InputStream inputStream, Date dataVencimentoFatura)
        ...
        String fileLocation = path.substring(0, path.length() - 1) + "fatura_cartao.xlsx";
